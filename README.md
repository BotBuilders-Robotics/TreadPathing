> **This branch is an experiment, not a release.** It carries the tank library exactly as
> published, plus `experimental/` — a holonomic (mecanum) follower built to find out what
> supporting mecanum would cost, and a path planner that can draw for either drivetrain.
> Nothing in `experimental/` is part of a release, and the `main` branch is what to clone if
> you want the library. Start at [experimental/README.md](experimental/README.md).
