...
public final class SplinePath {
    ...
    /**
     * Arc length at which segment {@code index} begins, so the distance to each waypoint is
     * {@code segmentStart(i)}. Added for the holonomic experiment, whose heading plans are
     * written in terms of the waypoints a team typed rather than the samples underneath.
     */
    public double segmentStart(int index) {
        return segmentStart[MathUtil.clamp(index, 0, segments.length - 1)];
    }
